...
exports.getAll = async (req, res, next) => {
  try {
    ...
    if (req.shopId && Category.getAttributes  && Category.getAttributes .ShopId) {
      query.ShopId = req.shopId;
    }
    const records = await Category.findAll({ where: query });
...
exports.update = async (req, res, next) => {
  try {
    let existing = await Category.findByPk(req.params.id);
    if (!existing) {
      return ApiResponse.error(res, 'Record not found', 404);
    }
