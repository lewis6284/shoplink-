...
exports.getAll = async (req, res, next) => {
  try {
    ...
    if (req.shopId && Expense.getAttributes  && Expense.getAttributes .ShopId) {
      query.ShopId = req.shopId;
    }
    const records = await Expense.findAll({ where: query });
...
exports.update = async (req, res, next) => {
  try {
    let existing = await Expense.findByPk(req.params.id);
    if (!existing) {
      return ApiResponse.error(res, 'Record not found', 404);
    }
