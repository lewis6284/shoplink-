...
exports.getAll = async (req, res, next) => {
  try {
    ...
    if (req.shopId && Unit.getAttributes  && Unit.getAttributes .ShopId) {
      query.ShopId = req.shopId;
    }
    const records = await Unit.findAll({ where: query });
...
exports.create = async (req, res, next) => {
  try {
    ...
    const record = await Unit.create(data);
...
exports.update = async (req, res, next) => {
  try {
    let existing = await Unit.findByPk(req.params.id);
    if (!existing) {
      return ApiResponse.error(res, 'Record not found', 404);
    }
