...
exports.getAll = async (req, res, next) => {
  try {
    ...
    if (req.shopId && Shop.getAttributes  && Shop.getAttributes .ShopId) {
      query.ShopId = req.shopId;
    }
    const records = await Shop.findAll({ where: query });
...
exports.create = async (req, res, next) => {
  try {
    ...
    const record = await Shop.create(data);
...
exports.update = async (req, res, next) => {
  try {
    let existing = await Shop.findByPk(req.params.id);
    if (!existing) {
      return ApiResponse.error(res, 'Record not found', 404);
    }
